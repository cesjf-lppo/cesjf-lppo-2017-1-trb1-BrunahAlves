...
<html>
    ...
    <body>
        ...
        <form method="post">
            <div><input type="hidden" name="id" value="${equipamento.id}"/><label>ID: ${equipamento.id}</label></div>
            <div><label>SÉRIE: ${equipamento.serie}</label></div>
            <div><label>LOCAL: <input type="text" name="local" value="${equipamento.local}"</label></div>
            <div><label>DESCRIÇÃO: ${equipamento.descricao}</label></div>
            <div><select name="estado" value="${equipamento.estado}">
